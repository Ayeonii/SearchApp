...
public class HttpAPIManager: NSObject {
    ...
    private class func call<T>(api: String, method: Alamofire.HTTPMethod, parameters:[String: Any]?, headers:HTTPHeaders, responseClass:T.Type, completion: @escaping (_ result: T?, _ error: NSError?) -> Void) where T: Decodable {
        ...
        request.responseData { (responseData) in
            DispatchQueue.global().async {
                ...
                if response.statusCode > 199 && response.statusCode < 300 {
                    ...
                    DispatchQueue.main.async {
                        completion(nil, NSError(domain: "LCIOS", code: response.statusCode, userInfo: nil))
                    }
                }
            }
        }
